  ...
  const badges = [
    {
      ...
      code: 'early-bird',
      condition: async (user, task) =>
        dayjs(task.completedAt).hour() < 12 &&
        (await getTasksDoneTodayCount({ userId: user.id })) >= 3,
    },
    {
      // 'Finish 2 tasks after 10pm'
      code: 'night-owl',
      condition: async (user, task) =>
        dayjs(task.completedAt).hour() > 22 &&
        (await getTasksDoneTodayCount({ userId: user.id, after: 22 })) >= 2,
    },
    {
      // 'Complete 5 tasks in one day'
      code: 'busy-bee',
      condition: async (user, task) =>
        (await getTasksDoneTodayCount({ userId: user.id })) >= 5,
    },
    {
      // 'Complete 10 tasks in one day'
      code: 'over-achiever',
      condition: async (user, task) =>
        (await getTasksDoneTodayCount({ userId: user.id })) >= 10,
    },
    {
      // 'Complete 20 tasks in one day'
      code: 'workaholic',
      condition: async (user, task) =>
        (await getTasksDoneTodayCount({ userId: user.id })) >= 20,
    },
    {
      // 'Complete your first task'
      code: 'first-task',
      condition: async (user, task) =>
        (await prisma.task.count({ where: { userId: user.id } })) === 1,
    },
    {
      // 'Complete 100 tasks'
      code: '100-tasks',
      condition: async (user, task) =>
        (await prisma.task.count({ where: { userId: user.id } })) >= 100,
    },
    {
      // 'Complete 500 tasks'
      code: '500-tasks',
      condition: async (user, task) =>
        (await prisma.task.count({ where: { userId: user.id } })) >= 500,
    },
    {
      // 'Complete 1000 tasks'
      code: '1000-tasks',
      condition: async (user, task) =>
        (await prisma.task.count({ where: { userId: user.id } })) >= 1000,
    },
    {
      // 'Complete 5000 tasks'
      code: '5000-tasks',
      condition: async (user, task) =>
        (await prisma.task.count({ where: { userId: user.id } })) >= 5000,
    },
    {
      // 'Complete 10000 tasks'
      code: '10000-tasks',
      condition: async (user, task) =>
        (await prisma.task.count({ where: { userId: user.id } })) >= 10000,
    },
